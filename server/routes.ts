...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  try {
    // Clean up any converso activity accidentally given PROG_BAUTISMO_ITEMS, but keep prog_flyer
    await db.execute(sql`
      DELETE FROM activity_checklist_items
      WHERE (item_key LIKE 'prog_%' OR item_key LIKE 'coord_%')
        AND item_key != 'prog_flyer'
        AND activity_id IN (
  ...
  app.patch("/api/sacramental-meetings/:id/vote-result", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const [current] = await db.select({ voteResults: sacramentalMeetingsTable.voteResults })
        .from(sacramentalMeetingsTable)
        .where(eq(sacramentalMeetingsTable.id, meetingId));
      const existing = (current?.voteResults as Record<string, any>) ?? {};
      const updated = {
        ...existing,
        [type]: {
          result,
          opponentName: opponentName || null,
          opposedTo: opposedTo || null,
          votedAt: new Date().toISOString(),
        },
      };
      await db.update(sacramentalMeetingsTable)
        .set({ voteResults: updated })
        .where(eq(sacramentalMeetingsTable.id, meetingId));
      ...
      await sendVoteResultEmail({
        ...
        meetingDate, type, items, result,
        opponentName: opponentName || undefined,
        opposedTo:    opposedTo    || undefined,
      });
      res.json({ ok: true });
    } catch (err) {
...
<body>
  ...
  // ── POST /api/activities/:id/publish-baptism-program ─────────────────────
  // Creates/updates a baptism_services record from the activity section_data
  // and publishes the public program link (for niño inscrito activities)
  app.post("/api/activities/:id/publish-baptism-program", requireAuth, async (req: Request, res: Response) => {
    try {
      const user = (req as any).user;
      const activityId = req.params.id;
      const canPublish = ["obispo", "consejero_obispo", "secretario", "secretario_ejecutivo"].includes(user.role);
      if (!canPublish) return res.status(403).json({ error: "Sin permiso" });
      ...
      // Map section_data → baptism_program_items
      const PROG_MAP = [
        { key: "prog_preside",          type: "preside",                order: 1  },
        { key: "prog_dirige",           type: "dirige",                 order: 2  },
        { key: "prog_himno_apertura",   type: "primer_himno",           order: 3  },
      ...
      for (const { key, type, order } of PROG_MAP) {
        const value = (sectionData[key] ?? "").trim();
        if (!value) continue;
        let hymnId: string | null = null;
        if (key === "prog_himno_apertura" || key === "prog_himno_cierre") {
          const m = value.match(/^(\d+)/);
