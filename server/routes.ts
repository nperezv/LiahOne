...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/interviews", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      // Try to find user by name (fuzzy match)
      ...
      if (personName) {
        ...
        // Exact match first
        let foundUser = users.find((u: any) => u.name.toLowerCase() === normalizedInput);
      ...
      // Get the requesting user to check if they're an organization member
      const requestingUser = await storage.getUser(req.session.userId!);
      const isOrgMember = ["presidente_organizacion", "secretario_organizacion", "consejero_organizacion"].includes(requestingUser?.role || "");
      
      // If org member requests an interview, notify all obispado members
      if (isOrgMember) {
        const users = await storage.getAllUsers();
        const obispadoMembers = users.filter((u: any) => 
          u.role === "obispo" || u.role === "consejero_obispo"
        );
        ...
        for (const member of obispadoMembers) {
          const notification = await storage.createNotification({
            ...
            description: `${requestingUser?.name || personName} solicita una entrevista para el ${new Date(interview.date).toLocaleDateString("es-ES")}`,
            relatedId: interview.id,
            isRead: false,
          });
          ...
          if (isPushConfigured()) {
            await sendPushNotification(member.id, {
              title: "Nueva Solicitud de Entrevista",
              body: `${requestingUser?.name || personName} solicita una entrevista`,
              url: "/interviews",
              notificationId: notification.id,
            });
          }
  ...
  app.put("/api/interviews/:id", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      if (personName) {
        ...
        if (foundUser) {
          updateData.assignedToId = foundUser.id;
        } else {
          updateData.assignedToId = null;
        }
      }

      const interviewData = insertInterviewSchema.partial().parse(updateData);
  ...
  app.delete("/api/interviews/:id", requireAuth, requireRole("obispo"), async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
